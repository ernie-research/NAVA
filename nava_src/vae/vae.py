...
class DiffusersVAEAdapter(nn.Module):
    """
    让自定义 KL-VAE 看起来像 diffusers.AutoencoderKL：
    - encode(x) 返回对象，含 .latent_dist（且有 .sample()）
    - decode(z) 返回对象，含 .sample
    - 提供 .config.scaling_factor
    - 暴露 .dtype / .device
    """
    def __init__(self, inner_vae: nn.Module, scaling_factor: float = 0.18215):
        super().__init__()
        self.inner = inner_vae
        # 伪 config，给 pipeline 用
        self.config = SimpleNamespace(scaling_factor=float(scaling_factor))
    ...
    def encode(self, x):
        posterior = self.inner.encode(x)              # 你的 DiagonalGaussianDistribution
        return SimpleNamespace(latent_dist=posterior) # 与 diffusers 对齐

    @torch.no_grad()
    def decode(self, z):
        img = self.inner.decode(z)
        return SimpleNamespace(sample=img)            # 与 diffusers 对齐