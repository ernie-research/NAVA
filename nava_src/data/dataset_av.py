    ...
    def _build_out_aud(self, obj: dict):
        ...
        try:
            ...
            for sample_audio_info, sample_text_info in zip(audio_splits_info, text_list):
                ...
                # encode audio
                sample_spk_embs = []
                bos_url = sample_audio_info["bos_url"]
    ...
    def _build_out_img(self, obj: dict):
        ...
        try:
            ...
            for sample_image_info, sample_text_info in zip(image_info, text_list):
                ...
                bos_url = sample_image_info["bos_url"]
                ...
                image_latents = self.image_vae_server.encode(
                    bos_url, rank=self.dist_info.world_rank
                ).latent_dist.sample()
                if self._perf:
    ...
    def _build_out_vid(self, obj: dict):
        ...
        try:
            ...
            for sample_video_info, sample_text_info in zip(video_info, text_list):
                ...
                bos_url = sample_video_info["bos_url"]
        ...
        try:
            ...
            for s_idx, (sample_video_info, sample_text_info) in enumerate(zip(video_info, text_list)):
                ...
                if self.split_wav_mode:
                    ...
                    audio_bos_url = audio_info[s_idx]["bos_url"]
                else:
                    audio_bos_url = bos_url
                video_height = sample_video_info["image_height"]
                video_width = sample_video_info["image_width"]
                video_duration = sample_video_info["duration"]
                video_frames = int(video_duration * self.video_fps)
                ...
                if audio_length > audio_latents.shape[0]:
                    audio_latents = torch.cat([audio_latents, torch.zeros(size=(audio_length-audio_latents.shape[0], audio_latents.shape[1]))], dim=0)
                else:
...
def main():
    ...
    cfg_audio = yaml.safe_load(open("configs/baseline_t2a_demo.yaml", "r"))
    cfg_video = yaml.safe_load(open("configs/baseline_t2v_demo.yaml", "r"))
    cfg_image = yaml.safe_load(open("configs/baseline_t2i_demo.yaml", "r"))
    cfg_audio_video = yaml.safe_load(open("configs/baseline_t2av_spk.yaml", "r"))

    check_modality = "audio_video"
    audio_vae_server, image_vae_server, video_vae_server = None, None, None

    if "audio" in check_modality:
        cfg = cfg_audio
    ...
    if "video" in check_modality:
        cfg = cfg_video
        video_vae_server = VAEServerAdapter(
            modality="video",
            scaling_factor=cfg["data"].get("video_scaling_factor", 1.0),
            shift_factor=cfg["data"].get("shift_factor", 0.0),
            resolution=cfg["image_size"],
            zlib_mode=cfg["data"].get("zlib_mode", True),
            server_list="nava_src/data/server/video_server.list",
        )
    if check_modality == "image":
        cfg = cfg_image
        image_vae_server = VAEServerAdapter(
            modality="image",
            scaling_factor=cfg["data"].get("image_scaling_factor", 1.0),
            shift_factor=cfg["data"].get("image_shift_factor", 0.0),
            resolution=cfg["image_size"],
            zlib_mode=cfg["data"].get("zlib_mode", True),
            server_list="nava_src/data/server/video_server.list",
        )
    if check_modality == "audio_video":
        cfg = cfg_audio_video
    ...
    for step, batch in enumerate(dl):
        ...
        print(f"step {step}")
        print("  data_state:", batch["data_state"])  # [B, 1+num_sources]
        print("-" * 40)
