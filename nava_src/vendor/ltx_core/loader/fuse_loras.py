...
from nava_src.vendor.ltx_core.loader.primitives import LoraStateDictWithStrength, StateDict


def calculate_weight_float8(*args, **kwargs):
    raise NotImplementedError(
        "fp8 LoRA fusion is unused by NAVA's vendored audio VAE path; "
        "the upstream `ltx_core.quantization` module was intentionally not vendored."
    )


def quantize_weight_to_fp8_per_tensor(*args, **kwargs):
    raise NotImplementedError(
        "fp8 LoRA fusion is unused by NAVA's vendored audio VAE path; "
        "the upstream `ltx_core.quantization` module was intentionally not vendored."
    )
