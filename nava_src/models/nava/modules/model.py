...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.gradient_checkpointing_offload = False
    ...
    def set_rope_params(self):
        ...
        if self.is_audio_type:
            ...
            if self.cross_1d_rope:
                self.cross_1d_freqs = rope_params(1024, d, freqs_scaling=self.temporal_rope_scaling_factor)
            else:
                self.cross_1d_freqs = None
        else:
    ...
    def prepare_transformer_block_kwargs(
        ...
        if self.freqs.device != device:
            ...
            if self.cross_1d_freqs is not None:
                self.cross_1d_freqs = self.cross_1d_freqs.to(device)
        ...
        kwargs = dict(
            ...
            cross_1d_freqs=self.cross_1d_freqs)
    ...
    def forward(
        ...
        for block in self.blocks:
            x = gradient_checkpoint_forward(
                    block,
                    use_gradient_checkpointing=(self.training and self.gradient_checkpointing),
                    use_gradient_checkpointing_offload=(self.training and self.gradient_checkpointing_offload),
                    x=x,
                    **kwargs
                )
